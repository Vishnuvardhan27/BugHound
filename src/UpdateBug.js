...
function UpdateBug() {
  ...
  useEffect(() => {
    console.log(state)
    setProblem(state.record.problem);
    setProblemSummary(state.record.problem_summary);
    setSeverity(state.record.severity);
    setReportType(state.record.report_type);
    setSuggestedFix(state.record.suggested_fix);
    setReproducible(state.record.reproducible);
    //setReportedDate(state.record.reported_date);
    setReportedBy(state.record.reported_by);
    setAssignedTo(state.record.assigned_to);
    setFunctionalArea(state.record.functional_area);
    setComments(state.record.comments);
    setStatus(state.record.status);
    setPriority(state.record.priority);
    setResolution(state.record.resolution);
    setResolutionVersion(state.record.resolution_version);
    setResolvedBy(state.record.resolved_by);
    //setResolvedDate(state.record.reported_date);
    //setTestedDate(state.record.tested_date);
    setTestedBy(state.record.tested_by);
    setProgram(state.record.program);
    setTreatedAsDeferred(state.record.treated_as_deferred);
    setBugId(state.record.bug_id)
  },[state]);
  ...
  function handleUpdateBug(e) {
    ...
    fetch(
      `http://localhost:3005/bugs/update/${bugId}`,
      {
        method: "PUT",
